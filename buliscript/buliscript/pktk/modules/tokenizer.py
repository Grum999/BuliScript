...
class TokenizerRule(object):
    ...
    def __init__(self, type, regex, description=None, autoCompletion=None, autoCompletionChar=None, caseInsensitive=True, onInitValue=None):
            ...
            for textIndex in range(len(match.capturedTexts())):
                ...
                for rule in self.__rules:
                    ...
                    if rule.regEx(True).match(value).hasMatch():
                        ...
                        if indent!=0 and (re.search('^\s*$', tokenText) is None) and token.column()==1:
